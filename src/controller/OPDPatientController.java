...
public class OPDPatientController {
    ...
    public boolean addPatient(Patient patient) throws SQLException, ClassNotFoundException {
        ...
        String query = "INSERT INTO `Opd Counter` VALUES (?,?,?,?,?,?)";
    ...
    public PtientDetailsInSections getPatient(String id) throws SQLException, ClassNotFoundException {
        PreparedStatement stm = DbConnection.getInstance().getConnection().prepareStatement("SELECT  * FROM `opd counter` WHERE PatientID=? ");
        stm.setObject(1,id);
