...
public class PatientInENTClinic {
    ...
    public boolean addPatientDirectly(ClinicPatient patient){
        ...
        try {
            ...
            PreparedStatement stm = DbConnection.getInstance().getConnection().prepareStatement("INSERT INTO  `ent clinc` VALUES (?,?,?,?,?,?,?,?,?)");
    ...
    public boolean deletePatientOnlyClinic(String id) throws SQLException, ClassNotFoundException {
        Connection connection = DbConnection.getInstance().getConnection();
        PreparedStatement stm = connection.prepareStatement("DELETE FROM `ent clinc` WHERE PatientID =?");
        stm.setObject(1,id);
        return stm.executeUpdate()>0;
    }


    public boolean deletePatientOnClinic(String text) throws SQLException, ClassNotFoundException {
        Connection connection = DbConnection.getInstance().getConnection();
        PreparedStatement stm = connection.prepareStatement("DELETE FROM `ent clinc` WHERE PatientID =?");
        stm.setObject(1,text);
        if (stm.executeUpdate()>0) {
            if (delePatientOnHistory(text)) {
                return true;
            }else {
                return false;
            }
    ...
    public boolean updatePatient(ClinicPatient patient) {
        ...
        try {
            connection = DbConnection.getInstance().getConnection();
            connection.setAutoCommit(false);
            PreparedStatement stm = connection.prepareStatement("UPDATE `ent clinc` SET ClinicDate=?,PatientName=?, PatientAge=?,  Reson=?, Illness=?, Reports=?, Medicine=?, DoctorID=? WHERE PatientID=?");
            stm.setObject(1,patient.getDate());
    ...
    public SearchPatien getPatient(String id) throws SQLException, ClassNotFoundException {
        Connection connection = DbConnection.getInstance().getConnection();
        PreparedStatement stm = connection.prepareStatement("SELECT * FROM `ent clinc` WHERE PatientID =?");
        stm.setObject(1,id);
        ResultSet set = stm.executeQuery();
        if (set.next()) {
