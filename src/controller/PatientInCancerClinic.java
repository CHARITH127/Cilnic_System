...
public class PatientInCancerClinic {
    ...
    public boolean addPatientDirectly(ClinicPatient patient){
        ...
        try {
            ...
            stm = DbConnection.getInstance().getConnection().prepareStatement("INSERT INTO  `cancer clinc` VALUES (?,?,?,?,?,?,?,?,?)");
    ...
    public  boolean addPatientInToTheClinic(ClinicPatient patient){
        try {
            PreparedStatement stm = DbConnection.getInstance().getConnection().prepareStatement("INSERT INTO  `cancer clinc` VALUES (?,?,?,?,?,?,?,?,?)");
            stm.setObject(1,patient.getDate());
            stm.setObject(2,patient.getPatientID());
            stm.setObject(3,patient.getPatientName());
            stm.setObject(4,patient.getPatientAge());
    ...
    public SearchPatien getPatient(String id) throws SQLException, ClassNotFoundException {
        Connection connection = DbConnection.getInstance().getConnection();
        PreparedStatement stm = connection.prepareStatement("SELECT * FROM `cancer clinc` WHERE PatientID =?");
        stm.setObject(1,id);
    ...
    public boolean updatePatient(ClinicPatient patient) {
        ...
        try {
            connection = DbConnection.getInstance().getConnection();
            connection.setAutoCommit(false);
            PreparedStatement stm = connection.prepareStatement("UPDATE `cancer clinc` SET ClinicDate=?,PatientName=?, PatientAge=?,  Reson=?, Illness=?, Reports=?, Medicine=?, DoctorID=? WHERE PatientID=?");
            stm.setObject(1,patient.getDate());
    ...
    public boolean deletePatientOnClinic(String id) throws SQLException, ClassNotFoundException {
        Connection connection = DbConnection.getInstance().getConnection();
        PreparedStatement stm = connection.prepareStatement("DELETE FROM `cancer clinc` WHERE PatientID =?");
        stm.setObject(1,id);
        if (stm.executeUpdate()>0) {
